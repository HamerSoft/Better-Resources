...
{
    public class AsynchronousInitialisation : MonoBehaviour
    {
        private async void Start()
        {
            ...
            / Note: the event subscription is not really needed since we are awaiting init
            */
            BetterResources.Initialized += BetterResourcesOnInitialized_Handler;
            // create cancellation token to have control over when and how the task is cancelled
            var cancellationTokenSource = new CancellationTokenSource();
            // Initialize BetterResources.
            await BetterResources.InitializeAsync(null, cancellationTokenSource.Token);
            ...
            var myMaterial = await BetterResources.LoadAsync(myMaterialResource);
